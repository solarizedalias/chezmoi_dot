...
mp.observe_property('aid', 'none', function(_, id)
  if id and id ~= 'no' then
    -- print(string.format('id = %s aid = %s', tostring(id), tostring(aid)))
    aid = id
  end
end)
...
mp.add_key_binding('Ctrl+a', 'cycle_adevice', function()
...
mp.add_key_binding('Ctrl+b', 'cycleBack_adevice', function()
  deviceList = mp.get_property_native('audio-device-list')
